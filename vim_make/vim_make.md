...
Ниже тесты, с помощью которых я проверял, решает ли код выше поставленную задачу. Сначала добавил пример из книги и пару своих примеров в виде массива пар строк в формате "ввод" и "ожидаемый возврат". Для перезапуска тестирования выполнял команду `:wa | !./compressString_test.py`. Можно перенести выполнение внешней команды в _makefile_ и превратить команду в `wa | make`. На тему юнит-тестирования в _Python_ можно почитать [официальную документацию][7] или бесплатную [книгу][6].
...
Подобным образом я проверял возврат функции, содержимое вывода на экран, перехватывал исключения. Интересная функция `assertAlmostEqual(places=4, <...>)` сравнивает числа с плавающей точкой с заданной точностью.

Для упрощения упрощения тестирования решения одной задачи за другой я копировал файл для для тестирования, в котором подставлял нужные:
 * имя импортируемого модуля с использованием псевдонима - `import smth.func as testFunc`
 * списки входных и проверяемых данных

Для того, чтобы протестировать функцию, принимающую несколько параметров, я на первом месте ставил массив, а при передаче в функцию разворачивал его.
